...
for (street in streets) {
    if (street == "AVENIDA BOA VIAGEM - ENTRE OS NRS. 6114 E 5888") {
        datasubset <- dataset$Fluxo[dataset$Logradouro == street][651:1322]
    } else if (street == "AVENIDA BOA VIAGEM - TERCEIRO JARDIM") {
        datasubset <- dataset$Fluxo[dataset$Logradouro == street][662:1329]
    } else if (street == "AV. GOV. AGAMENON MAGALHAES - PROX. VIADUTO PRES. MEDICI") {
        datasubset <- dataset$Fluxo[dataset$Logradouro == street][668:1338]
    } else if (street == "AV. MAL. MASCARENHAS DE MORAES, AEROP BAIRRO IMBIRIBEIRA") {
        datasubset <- dataset$Fluxo[dataset$Logradouro == street][1:670]
    } else {
        datasubset <- dataset$Fluxo[dataset$Logradouro == street]
    }
    size <- length(datasubset)
    while (size < 672) {
        datasubset <- c(datasubset, NA)
        size <- size + 1
    }
    sample <- list(datasubset)
